...
In RAFT, we prepare the training data such that each data point contains a question ($Q$), a set of documents ($D_k$), and a corresponding Chain-of-though style answer, $ A*$, generated from one of the document, $ D_*$ (oracle document) while the non-relevant documents are ‘distractors’ ($D_i$).
- Train:
